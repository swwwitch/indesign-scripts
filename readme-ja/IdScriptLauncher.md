...
- 「検索条件を記憶」をONにすると、キーワード・「サブディレクトリを含む」・「フルパス」・左右のリストの選択を、InDesignを終了するまでのあいだ引き継ぐ（初期値はON）
...
- v1.1.0（2026-09-20）：「検索条件を記憶」を追加。ONのあいだ、InDesignのセッション中は前回のキーワードやリストの選択を引き継ぐ
- v1.0.2（2026-09-20）：件数に関わらず常に出すキーワードボタンを追加（font）
