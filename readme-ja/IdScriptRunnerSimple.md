# ExtendScript ファイルを選んで実行（最小構成）

[![Direct](https://img.shields.io/badge/Direct%20Link-IdScriptRunnerSimple.jsx-ffcc00.svg)](https://github.com/swwwitch/indesign-scripts/blob/main/jsx/runner/IdScriptRunnerSimple.jsx)

[![English](https://img.shields.io/badge/README-English-4b8bbe.svg)](https://github.com/swwwitch/indesign-scripts/blob/main/readme-en/IdScriptRunnerSimple.md)

[![Direct](https://img.shields.io/badge/Back%20to%20home-All%20scripts-cccccc.svg)](https://github.com/swwwitch/indesign-scripts/blob/main/README.md)

---

ExtendScript ファイルを 1 つ選んで実行するだけの最小構成のランチャーです。

## 主な機能

- ファイル選択ダイアログから実行するスクリプトを指定
- 選択されたファイルをそのまま実行

## 使い方

1. スクリプトを実行する
2. 実行したい ExtendScript ファイルを選ぶ

## 制限事項・メモ

- エラー処理や拡張子チェックはありません。必要な場合は IdScriptRunner.jsx を使ってください。

## スクリプト情報

| 項目 | 内容 |
| --- | --- |
| ファイル | `jsx/runner/IdScriptRunnerSimple.jsx` |
| バージョン | v1.0 |
| 作者 | Masahiro Takano (@swwwitch) |
| 初回リリース | 2026-04-17 |
| 最終更新 | 2026-04-17 |

## ライセンス

MIT License — <http://opensource.org/licenses/mit-license.php>
