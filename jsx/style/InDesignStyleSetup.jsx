#target indesign
    ...
    **その他**
    ...
    v1.2.0
    - basestyle グループに body-text / heading / base-toc スタイルを追加し、継承の基準を再編：base-regex → body-text →（p / ul-li / ol-li / p.caption・表セル th / td）、base-regex → heading → h1〜h6、basestyle の base-toc → toc-title / toc-h1〜toc-h3
    - body-text に カーニング「和文等幅」・均等配置（最終行左揃え）、heading に カーニング「メトリクス」・左揃え を設定（h1〜h6 / 本文系は basedOn で継承。p への直接の行揃え指定は廃止）
    - basestyle グループと base-regex スタイルを追加。共通 GREP（lang-US / no-break / inline-graphic）を base-regex に集約し、body-text / heading を base-regex 基準にして本文系・見出し系へ継承
    ...
    */
    ...
    (function () {
        ...
        function main() {
            ...
            function applyTableCellSettings(doc) {
                ...
                for (var tableBaseIndex = 0; tableBaseIndex < tableBaseStyleNames.length; tableBaseIndex++) {
                    ...
                    if (tableBaseStyle.isValid) {
                        if (APPLY_BASED_ON_RELATIONSHIPS && bodyTextStyle && bodyTextStyle.isValid) {
            ...
            /* toc-h3 の「次の段落を保持」を 0 に設定（base-toc を基準にしている場合は、継承した 2 を打ち消す。
               APPLY_BASED_ON_RELATIONSHIPS = false で基準を設定していない場合も 0 を設定するが、既定値が 0 のため実質変化なし）/
               Set keep-with-next = 0 on toc-h3 (when based on base-toc, cancels the inherited 2;
               with basedOn off it still sets 0, which is the default anyway) */
            ...
            /* 段落スタイルに正規表現スタイルを設定（同条件のものがあれば重複追加しない） /
               ...
               ※ 仕様: 既存の GREP ルールは削除しない（追加のみ）。スタイルに元からある／前回実行で付けた
                  GREP ルールはそのまま残し、本スクリプトの定義に無い旧ルールも消さない /
               NOTE (by design): existing GREP rules are never removed (add-only). Rules already on the
                  style — or added by a previous run — are kept, including old ones not in this definition.
